...
Task 4: complete — commits `269671f`, `c04f227`. Buyer-style Apple Silicon extraction, internal and external checksums, version/preset/scan smoke, before-after source hashes, focused adversarial scans, reproducibility, full tests/race/vet/format/diff checks, whole-diff review, and the private artifact manifest passed. Remote CI is green: run `32571165222` on `c04f227` passes both the theme/shop `verify` job and the CLI job, including the build and independent verification of all three private candidates and the Apple Silicon reproducibility comparison. `269671f` made six filesystem tests portable off macOS-only `/private/tmp`; `c04f227` moved the CI candidate and reproducibility output directories under `${RUNNER_TEMP}` so the clean-source packager is satisfied without being weakened. Candidates were rebuilt from clean `c04f227` and the manifest hashes replaced — the earlier `dc49778` hashes describe no current candidate. Matching-host execution of the packaged Intel/Linux executables remains an external unverified gate; publication remains blocked by the plan's owner-controlled legal, payment, download, and smoke-test gates.
