...
Later on 2026-08-24, following Gabriel's explicit instruction, Gumroad's refund selector was changed from the historical 14-day guarantee to `No refunds allowed`. The customer-facing description now says there are no voluntary change-of-mind refunds after download access begins while mandatory consumer rights remain unaffected. The fine print preserves any withdrawal right that has not legally expired and statutory remedies for defective or non-conforming digital content, and directs customers with a materially defective or misdescribed download to reply to the receipt for investigation and an appropriate remedy. Gumroad returned `Changes saved!` and moved to the Content tab. A full reload retained the sole `Audio Delivery Preflight 0.1.0` delivery folder and still exposed `Publish and continue`; reopening Product read back EUR 24, the new description, the selected no-refunds policy, and the full fine print with no remaining current 14-day promise. The preview still stated that the product was not currently for sale. Gumroad also reported that no payout method was connected and publication was unavailable. No payout, seller, tax, receipt, attachment, purchase, publication, customer communication, or promotion action occurred.

### 2026-08-24 exact replacement GUI happy path

Gabriel clicked Start Scan in the already-open exact app extracted at `/private/tmp/audio-preflight-manual-qa.RxU9WL`. Computer Use then reconnected by the exact full app path and directly read a General Audio `Ready` result for `Sample Delivery Package`, the technical-only boundary statement, six inventory entries, zero findings, and zero role assignments. The findings view showed no matching findings; the role-assignment view correctly reported no successful role assignments because General Audio defines no roles; and the inventory view exposed Artwork, `Artwork/Cover.png`, Credits, `Credits/credits.md`, Masters, and `Masters/Main Master.wav`.

The exact packaged GUI exported all three report types through separate native save panels into `/private/tmp/audio-preflight-manual-qa.RxU9WL/gui-reports` and returned visible confirmations for Accessible HTML, Versioned JSON, and SHA-256 checksums. The resulting evidence was:

- HTML: 3,825 bytes, SHA-256 `289a69e04217a0a81657416952db9fac2c5c80f3205d5561c73018bbcb5973e3`;
- JSON: 6,304 bytes, SHA-256 `1ba4cc9b19bc85f9fb3e36cbbcc19ae404faa70f2bb55f7d3fd42df48ce97401`;
- checksum manifest: 259 bytes, SHA-256 `fcd8b7ee0547bb9e36850fe3a4f70cc48c7c22cabfbada8272b21aa35a7615a6`.

The JSON records schema 1.0, app/engine 0.1.0, selected folder `Sample Delivery Package`, preset `general-audio`, overall status `ready`, six inventory entries, zero findings, and zero role assignments. The HTML records `Ready`, General Audio, and no findings. The GUI checksum manifest verified the exact packaged sample. The artwork, credits, and WAV retained the same SHA-256 values, byte sizes, mtimes, and modes as their pre-scan baseline and the earlier exact-package CLI evidence.

Digital Release and Stereo Premaster were also selected and read back in the exact GUI. Entering Custom and some later view transitions detached the Computer Use pipe again without a matching crash report, so Custom GUI, drag/drop, manual cancellation, repeated-finding details, and keyboard/VoiceOver checks are not inferred. The exact packaged CLI independently returned `ready` for all four presets: General Audio with zero roles, Stereo Premaster with one role, Digital Release with three roles, and Custom with zero roles; every run reported six inventory entries, zero errors, and zero warnings. A fresh focused cancellation run executed seven tests with zero failures, covering incomplete-not-ready settlement, immediate non-cooperative cancellation, enumeration, checksum, audio, image, and scan-service propagation and cleanup.

The exact app showed no network socket over 30 polls during a 15-second idle observation. The exact packaged CLI was separately polled every 0.1 seconds throughout a complete Digital Release scan and showed no network socket. This improves the local-only evidence but is not an active-GUI-scan firewall test. The packaged `.icns` was extracted and visually reviewed at 512, 32, and 16 pixels in the current dark appearance; it remained coherent and recognizable. Finder/Dock, light appearance, increased text, and assistive-technology checks remain open.

The current host is macOS 27.0 build `26A5416b`, not an independent macOS 14 host. An isolated checksum-verified copy was given a top-level quarantine attribute without changing system policy. `spctl` returned the same Code Signing subsystem internal error for both the known exact app and the isolated quarantined copy, and Finder/Computer Use did not provide reliable customer-warning evidence. No warning was bypassed. Browser-quarantine, manual-open, and independent macOS 14 customer-path gates therefore remain open. Gumroad remained unpublished throughout this QA.
...
| Local scan engine with no intended network request | `PreflightCore` and presentation targets contain no networking API; `PRIVACY.md` documents the boundary. The exact app showed no socket over 30 idle polls, and the exact packaged CLI showed none at 0.1-second polling throughout a Digital Release scan. Active-GUI-scan firewall/offline validation remains open. |
...
| Accessible native workflow with explicit selection, editable requirements, scan, results, and export phases | App-model tests cover Custom/Digital Release editing state, bounded storage, typed validation, structural regex rejection, role assignments, and the workflow. The exact `41beb74` app now has a current General Audio GUI `Ready` pass, six-entry inventory inspection, and all three native report exports. Final-build drag/drop, manual cancellation, Custom GUI, repeated-finding details, keyboard-only, and assistive-technology checks remain open. |
...
- Complete the remaining app matrix against the final distributable build: Finder drag and drop, manual cancellation, Custom GUI, filters, and each repeated finding/detail. The General Audio `Ready` path, inventory, and all exports have passed.
...
- Observe or deny network access during an active final packaged GUI scan. Idle app and active packaged-CLI observations passed without sockets.
...
This closes the repeatable Universal bundle/archive assembly, durable-copy CLI verification, unpublished Gumroad attachment readback, and one exact packaged General Audio GUI scan/results/all-exports/source-immutability path for replacement source commit `41beb74` and archive SHA-256 `cb175ec7...9ebf`. Gumroad's sole remaining delivery folder was downloaded after cleanup and matched the same replacement checksum, but that seller-side readback is not a buyer checkout or verified customer delivery. Manual cancellation, drag/drop, Custom/repeated-finding/assistive-technology/appearance checks, the browser-quarantined customer workflow, and an independent macOS 14 installation still require testing. Developer ID signing and notarization are intentionally not applicable to the selected no-Apple-payment route. Legal acceptance, seller/tax review, and public publication remain open.
