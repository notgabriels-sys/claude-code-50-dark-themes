...
10. runs release-binary probes proving that a renamed AAC cannot satisfy the built-in lossless role and that a category-agnostic Custom role carrying audio-only constraints returns exact invalid-configuration exit code 3 before requirements or scan output.
...
| Inventory work is finite and never truncated into `ready` | Boundary regressions cover 50,000 total entries, depth 32, 20,000 names per directory, 4096 UTF-8 bytes per relative path, and 16 MiB aggregate relative-path text. Every over-limit resource maps to a specific visible `incomplete` result, including a limit reached during the post-scan inventory. |
...
| Deterministic exit codes and interrupted-scan behavior | CLI tests cover ready, warnings, errors, invalid configuration, incomplete scans, and export failures. The release invalid-role probe requires exact exit 3, the invalid-configuration message, and no requirement or scan output. |
