...
- the README custom-preset JSON imported through `--preset-file`, resolved before scanning, returned `ready` with exit code 0, did not print its private import path, and retained identical preset and fixture SHA-256 evidence;
...
| Optional embedded metadata has bounded deterministic presentation | Production scans optionally request AVFoundation common metadata and retain no more than 64 selected metadata-key groups while performing no more than 64 value loads. Keys are truncated to 128 UTF-8 bytes, values to 4096 UTF-8 bytes, and entries are admitted only while their deterministic sorted-key JSON aggregate stays within 32768 UTF-8 bytes. Inspector regressions cover bounded selection, value loading, UTF-8-safe truncation, aggregate admission, and deterministic ordering. AVFoundation may expose no common metadata, fail the load, or materialize a collection before caller-side selection; the scanner preserves an empty optional result rather than guessing. Credits/metadata documents remain filename-matched package files whose contents are not parsed. |
...
| Relative, private, stable reports | Intentional JSON schema-`1.0` key-set/privacy tests, HTML escaping/accessibility tests, checksum-manifest tests, and exact fixture report inspection cover explicit inspection/checksum states, role assignments, relative paths, and optional bounded embedded metadata in the app and HTML/JSON reports. |
