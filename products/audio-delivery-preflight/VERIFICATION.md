...
The exact replacement GUI workflow has not yet been exercised. The chooser and Review requirements evidence below belongs to the superseded `60b9638` app and is retained only as historical smoke evidence. Gumroad product `vddnq` also still contains the previously downloaded-and-verified `60b9638` pair at this point; keep it unpublished, upload the `41beb74` pair, download and verify `cb175ec7...9ebf`, and only then remove the superseded provider files.
