...
| Framework-provided embedded metadata is optional, bounded, deterministic, and visibly reportable | Audio metadata mapping enforces 128-byte UTF-8 keys, 4096-byte UTF-8 values, at most 64 selected items/value loads, and a 32768-byte sorted-key JSON aggregate. Combining-scalar, four-byte-scalar, duplicate-key, permutation, old-selection-boundary, and real tagged-M4A staging tests pass; app, HTML escaping, and explicit JSON-v1 report mapping cover presentation. Unsupported or unavailable metadata remains absent rather than guessed. |
...
| Relative, private, stable reports | Intentional JSON schema-`1.0` key-set/privacy tests, HTML escaping/accessibility tests, checksum-manifest tests, and exact fixture report inspection cover explicit inspection/checksum states, role assignments, relative paths, and optional measured metadata. |
