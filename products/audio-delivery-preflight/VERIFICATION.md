...
A fresh run of `scripts/verify-release-archive.sh` against the durable ZIP on 2026-08-23 revalidated the external checksum, ZIP structure, internal manifest, required package files, architecture set, and all three ad-hoc signatures before stopping at the live Gatekeeper comparison. The current host is macOS 27.0 beta build `26A5406e`; `spctl` returned `internal error in Code Signing subsystem` with exit 1 for the packaged app, Apple TextEdit, and `/usr/bin/true`, while `spctl --status` still reported assessments enabled. Unified logs also showed repeated `syspolicyd` quarantine-process initialization failures. This is a host-wide Gatekeeper-assessment outage, not evidence that the archive changed or became accepted. No security setting or policy service was altered. The exact packaged CLI checks that occur after the verifier's Gatekeeper comparison were then run directly: version `0.1.0`, the deterministic sample scan returned `ready`, HTML/JSON/checksum reports were created, all three sample hashes remained identical, the unsigned disclosure was present, and identity/private-path leakage scans were clean. The repository verifier remains literally non-zero on this host until `spctl` can produce a valid assessment again.

The signed-in Gumroad product `vddnq` was reconciled and read back on 2026-08-23. `Audio Delivery Preflight` remains **Unpublished** at EUR 24 with the 14-day money-back guarantee. Its `Audio Delivery Preflight 0.1.0` delivery folder contains exactly the cleanly named ZIP and 126-byte sidecar. Gumroad's folder download contained exactly those two files; the sidecar check returned `OK` for SHA-256 `be7a195b...6d04`. The superseded `9e96ef8c...4c64` ZIP and sidecar and the failed zero-byte upload were removed only after the replacement download was verified. The receipt message and `Download Audio Preflight` button were read back, and the public route explicitly says the product is not currently for sale with no purchase button. No publication, purchase, payment, customer email, or public promotion occurred.
...
This closes the repeatable Universal bundle/archive-assembly, durable-copy launch, and exact Gumroad attachment-readback portions of the release gate for source commit `60b9638` and archive SHA-256 `be7a195b...6d04`. The durable local copy and provider download both verify, and the exact durable app reaches Review requirements with the sample, but the exact GUI scan/export workflow, browser-quarantined customer workflow, and independent macOS 14 installation still require testing. Developer ID signing and notarization are intentionally not applicable to the selected no-Apple-payment route. Packaged-icon visual inspection, accessibility testing, customer-path quarantine behavior, legal acceptance, seller/tax review, and public publication remain open. A valid live Gatekeeper reassessment must also be repeated after the macOS 27 beta host's system-wide `spctl` failure clears.
