...
UNSAFE_REGEX_DIR="$VERIFY_TMP/unsafe-regex-source"
mkdir -p "$UNSAFE_REGEX_DIR"
/bin/cp "$AAC_FIXTURE" "$UNSAFE_REGEX_DIR/Main.m4a"
UNSAFE_REGEX_PRESET="$VERIFY_TMP/unsafe-regex-preset.json"
/usr/bin/tee "$UNSAFE_REGEX_PRESET" > /dev/null <<'JSON'
{
  "schemaVersion": "1.0",
  "identifier": "unsafe-regex-probe",
  "name": "Unsafe Regex Probe",
  "audio": {
    "requireConsistentSampleRate": false,
    "requireConsistentBitDepth": false,
    "requireConsistentChannelCount": false,
...
JSON
snapshot_directory "$UNSAFE_REGEX_DIR" "$VERIFY_TMP/unsafe-regex-before.tsv"
if "$PRODUCT_DIR/.build/release/audio-preflight" scan "$UNSAFE_REGEX_DIR" \
    --preset-file "$UNSAFE_REGEX_PRESET" \
    > "$VERIFY_TMP/unsafe-regex.stdout" \
    2> "$VERIFY_TMP/unsafe-regex.stderr"; then
    UNSAFE_REGEX_EXIT=0
else
    UNSAFE_REGEX_EXIT=$?
fi
snapshot_directory "$UNSAFE_REGEX_DIR" "$VERIFY_TMP/unsafe-regex-after.tsv"
/usr/bin/cmp "$VERIFY_TMP/unsafe-regex-before.tsv" "$VERIFY_TMP/unsafe-regex-after.tsv"
if (( UNSAFE_REGEX_EXIT != 3 )); then
    print -u2 -- "Unsafe imported-regex release probe returned exit $UNSAFE_REGEX_EXIT instead of 3."
    exit 1
fi
if ! /usr/bin/grep -F -q -- 'Invalid command or configuration.' "$VERIFY_TMP/unsafe-regex.stderr"; then
    print -u2 -- "Unsafe imported-regex release probe did not report invalid configuration."
    exit 1
fi
if /usr/bin/grep -E -q -- 'Resolved requirements|Scan summary:|Status:' "$VERIFY_TMP/unsafe-regex.stdout"; then
    print -u2 -- "Unsafe imported-regex release probe reached requirement or scan output."
    exit 1
fi
...
print -- "Adversarial release probes: renamed AAC exits 2; invalid Custom role exits $CUSTOM_AAC_EXIT; unsafe imported regex exits $UNSAFE_REGEX_EXIT; sources unchanged."
if [[ "$PACKAGE_TEST_SKIPPED" == true ]]; then
    print -- "Developer-ID-unsigned release packaging: deferred to the calling packager to prevent recursive verification."
else
    print -- "Developer-ID-unsigned release packaging: real Universal binaries, selected icon, ad-hoc signatures, provenance, immutable sample scan, archive paths, tamper rejection, and SHA-256 verified."
fi
