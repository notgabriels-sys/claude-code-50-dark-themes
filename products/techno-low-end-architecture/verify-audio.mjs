...
const packRoot = packRootArgument
  ...
  : join(root, "pack", "Techno_Low_End_Architecture_by_Hologram_People");
const approvalPath = join(root, "AUDIO_APPROVAL.json");
const archivePath = join(root, "dist", "Techno_Low_End_Architecture_by_Hologram_People.zip");
const reelPath = join(root, "qa", "TLEA_Audition_Reel.mp3");
const notes = ["C1", "Db1", "D1", "Eb1", "E1", "F1", "Gb1", "G1", "Ab1", "A1", "Bb1", "B1"];
const families = new Map([
  ["01_Sub_Pulses", { stem: "Sub", minimumDuration: 1.0, maximumDuration: 1.6, exactMono: true }],
  ["02_Saturated_Bass_Stabs", { stem: "Stab", minimumDuration: 1.2, maximumDuration: 2.0 }],
  ["03_Rumble_Tails", { stem: "Rumble", minimumDuration: 3.0, maximumDuration: 4.3 }],
  ["04_Low_Frequency_Motion", { stem: "Motion", minimumDuration: 2.4, maximumDuration: 3.5 }],
]);
...
function inspectPcm24(wav) {
  ...
  return {
    ...
    low_band_side_to_mid_db:
      lowSideRms > 0 && lowMidRms > 0 ? Number((20 * Math.log10(lowSideRms / lowMidRms)).toFixed(2)) : null,
...
for (const [folder, family] of families) {
  for (let noteIndex = 0; noteIndex < notes.length; noteIndex += 1) {
    ...
    const path = `${folder}/TLEA_${family.stem}_${note}_${number}.wav`;
    expectedPaths.push(path);
    expectedMetadata.set(path, { folder, family, note });
  }
}
...
try {
  ...
  if (manifest.product !== "Techno Low-End Architecture") failures.push("Manifest product identity is incorrect.");
  if (manifest.artist !== "Hologram People") failures.push("Manifest artist identity is incorrect.");
  if (manifestPaths.size !== 48) failures.push(`Manifest should contain 48 unique WAVs; found ${manifestPaths.size}.`);
  ...
  for (const entry of entries) {
    ...
    if (entry.family !== expected.folder) failures.push(`Manifest family is incorrect for ${entry.file}.`);
    if (entry.root_note !== expected.note) failures.push(`Manifest root note is incorrect for ${entry.file}.`);
...
for (const file of wavFiles) {
  ...
  if (pcm.stereo_correlation != null && pcm.stereo_correlation < -0.2) {
    failures.push(`${name}: stereo correlation ${pcm.stereo_correlation} is unsafe for this low-end product.`);
  }
  ...
  if (expected?.family.exactMono && pcm.maximum_stereo_difference > 0.000_000_12) {
    failures.push(`${name}: sub pulse is not sample-identical between channels.`);
  }
...
const report = {
  ...
  provisional_delivery_contract: {
    ...
    categories: Object.fromEntries([...families].map(([name]) => [name, 12])),
  },
  summary: {
    ...
    stereo_correlation_range:
      measurements.filter((measurement) => measurement.stereo_correlation != null).length > 0
        ? [
            Math.min(...measurements.filter((measurement) => measurement.stereo_correlation != null).map((measurement) => measurement.stereo_correlation)),
            Math.max(...measurements.filter((measurement) => measurement.stereo_correlation != null).map((measurement) => measurement.stereo_correlation)),
          ]
        : null,
  },
