...
// The gallery renders from an inline JS array, so no theme name exists in
// crawlable HTML. This ItemList block, injected into index.html between the
// markers below, is the crawlable form: name + deep link per theme, generated
// from the same source as themes.json so it cannot disagree with what ships.
// Free themes only, and deliberately price-free — see the storefront-exposure
// skill on why no price ever enters a machine-readable surface.
export const ITEMLIST_START = "<!-- theme-itemlist:generated by scripts/build-theme-index.mjs, do not edit -->";
export const ITEMLIST_END = "<!-- /theme-itemlist -->";

export function serializeThemeItemList(index) {
  const itemList = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: index.name,
    numberOfItems: index.count,
    itemListElement: index.themes.map((theme, position) => ({
      "@type": "ListItem",
      position: position + 1,
      name: theme.name,
      url: theme.url,
...
export function injectThemeItemList(html, index) {
  const start = html.indexOf(ITEMLIST_START);
  const end = html.indexOf(ITEMLIST_END);
  if (start === -1 || end === -1) {
    throw new Error(
      "index.html is missing the theme-itemlist markers; the crawlable theme " +
        "list has nowhere to live.",
    );
  }
  return (
    html.slice(0, start) +
    serializeThemeItemList(index) +
    html.slice(end + ITEMLIST_END.length)
  );
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  await writeFile(pageUrl, injectThemeItemList(html, index));
  console.log(`Built themes.json and the ItemList block with ${index.count} themes.`);
}
